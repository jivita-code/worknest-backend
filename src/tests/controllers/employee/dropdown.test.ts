...
describe("getEmployeesDropdown", () => {
  ...
  beforeEach(() => {
    ...
    next = jest.fn();
    (employeeService.getEmployeesDropdown as jest.Mock).mockReset();
  });
  ...
  test("should return employees dropdown successfully", async () => {
    ...
    (employeeService.getEmployeesDropdown as jest.Mock).mockResolvedValue(mockEmployees);

    await getEmployeesDropdown(req as Request, res as Response, next);
  ...
  test("should call next on service error", async () => {
    ...
    (req as any).user = { org_id: "org-123" };
    (employeeService.getEmployeesDropdown as jest.Mock).mockRejectedValue(error);

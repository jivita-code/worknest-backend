...
import { updateOrganization } from "../../../controllers/org.controller.js";
...
describe("updateOrganization", () => {
  ...
  beforeEach(() => {
    ...
    next = jest.fn();
    (orgService.updateOrganization as jest.Mock).mockReset();
  });
  ...
  test("should update organization successfully", async () => {
    ...
    req.body = updateData;
    (orgService.updateOrganization as jest.Mock).mockResolvedValue(updatedOrg);
  ...
  test("should call next on service error", async () => {
    ...
    (req as any).user = { org_id: "123e4567-e89b-12d3-a456-426614174000" };
    req.body = { name: "New Name" };
    (orgService.updateOrganization as jest.Mock).mockRejectedValue(error);
